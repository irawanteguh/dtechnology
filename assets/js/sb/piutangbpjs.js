...
function dataharian() {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            for (let i = 1; i <= 12; i++) {
                let key = i < 10 ? i : '' + i;
                $("#resultpiutangtabblnrsms" + key).html("");
                $("#resultpiutangtabblnrsiabm" + key).html("");
                $("#resultpiutangtabblnrst" + key).html("");
                $("#resultpiutangtabblnrmb" + key).html("");
            }
        },
            ...
            if(data.responCode === "00"){
                ...
                for (let i in result) {
                    ...
                    let htmlRS = `
                        <tr>
                            ...
                            <td>${row.ptd == 1 ? '<span class="badge bg-primary">Rawat Inap</span>' : row.ptd == 2 ? '<span class="badge bg-success">Rawat Jalan</span>' : ''}</td>
                            <td>${row.mrn}</td>
                            <td>${row.nama_pasien}</td>
                            <td>${row.nokartu}</td>
                            <td>${row.sep}</td>
                            <td>${row.dpjp}</td>
                            <td class='text-end pe-4'>${todesimal(row.tarif_inacbg)}</td>
                        </tr>
                    `;

                    $("#resultpiutangtabbln" + rsCode + keyMonth).append(htmlRS);
                ...
                for (let m = 1; m <= 12; m++) {
                    ...
                    if (groupRMB[key]) {
                            ...
                            for (let item of g.rows) {
                                rows.push(`
                                    <tr>
                                        ...
                                        <td>${item.ptd == 1 ? '<span class="badge bg-primary">Rawat Inap</span>' : item.ptd == 2 ? '<span class="badge bg-success">Rawat Jalan</span>' : ''}</td>
                                        <td>${item.mrn}</td>
                                        <td>${item.nama_pasien}</td>
                                        <td>${item.nokartu}</td>
                                        <td>${item.sep}</td>
                                        <td>${item.dpjp}</td>
                                        <td class='text-end pe-4'>${todesimal(item.tarif_inacbg)}</td>
                                    </tr>
                                `);
                        ...
                        `);
                        $("#resultpiutangtfoottabblnrmb" + key).html(rows.join(""));
                    }
                }
                ...
                for (let m = 1; m <= 12; m++) {
                    ...
                    for (let rsCode of ["rsms", "rsia", "rst"]) {
                        if (groupPerRS[rsCode][key]) {
                            ...
                            $("#resultpiutangfoottabbln" + rsCode + key).html(tfoot);
                        }
                    }
                }
            }
