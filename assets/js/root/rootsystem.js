...
function viewdocwithoutnote(btn) {
    var filename     = btn.getAttribute('data-dirfile');
    var filename     = filename.replace('/www/wwwroot/', 'http://');

    var viewfile = "<iframe src='" + filename + "#toolbar=1' width='100%' height='100%' type='application/pdf' id='view'></iframe>";
    $("#viewdoc").html(viewfile);
    $('#openInNewTabButton').data('filename', filename);
};

function viewdocwithnote(btn) {
    ...
    if(responsefile === 200){
        var viewfile = "<embed src='" + filename + "#toolbar=0&navpanes=0&scrollbar=0' width='100%' height='100%' type='application/pdf' id='view'>";
...
function viewdocdisposisiwithoutnote(btn) {
    var filename     = btn.getAttribute('data-dirfile');
    var filename     = filename.replace('/www/wwwroot/', 'http://');
    var responsefile = jQuery.ajax({url: filename,type: 'HEAD',async: false}).status;

    if(responsefile === 200){
        var viewfiledisposisi = "<embed src='" + filename + "#toolbar=0' width='100%' height='100%' id='view'>";

        $("#viewdoclembardisposisi").html(viewfiledisposisi);
        $('#openInNewTabButton').data('filename', filename);
    } else {
        var viewfile = `
            <div class='alert alert-dismissible bg-light-info border border-info border-3 border-dashed d-flex flex-column flex-sm-row w-100 p-5 mb-10 fa-fade'>
        ...
        $("#viewdoclembardisposisi").html(viewfile);
        $('#openInNewTabButton').data('filename', '');
    }
};