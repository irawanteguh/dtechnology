...
$("#modal_master_detail_spu").on('shown.bs.modal', function(event){
    ...
    detailbarangspu(datanopemesanan,"CTO");
});
...
function datapemesanan(startDate,endDate){
    $.ajax({
        url       : url+"index.php/logistiknew/approvalcto/datapemesanan",
                    ...
                    rows += "<td class='text-end'>";
                        rows +="<div class='btn-group' role='group'>";
                            ...
                            if(result[i].methodid==="9"){
                                if(result[i].status==="21"){
                                    rows +="<a class='dropdown-item btn btn-sm text-success' "+getvariabel+" datastatus='27' datavalidator='CTO' onclick='validasi($(this));'><i class='bi bi-check2-circle text-success'></i> Approved</a>";
                                    rows +="<a class='dropdown-item btn btn-sm text-danger' "+getvariabel+" datastatus='26' datavalidator='CTO' onclick='validasi($(this));'><i class='bi bi-trash-fill text-danger'></i> Decline</a>";
                                }

                                if(result[i].status==="27"){
                                    rows +="<a class='dropdown-item btn btn-sm text-info' "+getvariabel+" datastatus='21' datavalidator='CTO' onclick='validasi($(this));'><i class='bi bi-arrow-counterclockwise text-info'></i> Cancel Approved</a>";
                                }
                            }
