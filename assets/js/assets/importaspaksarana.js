...
$('#uploadForm').on('submit', function(e) {
    ...
    if (mode === 'upload') {
        ...
        $.ajax({
            url        : 'importaspaksarana/upload_file',
            type       : 'POST',
            data       : formData,
            contentType: false,
            processData: false,
            success    : function(resp) {
                ...
                if (result.status === 'success' && result.data.length > 0) {
                    let html = '<table class="table align-middle table-row-dashed fs-8 gy-2"><thead class="text-center"><tr class="fw-bolder text-muted bg-light align-middle">';
        ...
        $.ajax({
            url: 'importaspaksarana/import_to_db',
            type: 'POST',
