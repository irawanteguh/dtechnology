    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (let bulanKe = 1; bulanKe <= totalBulan; bulanKe++) {
                ...
                const idxForecast = (bulanKe - 1 + 12) % 12; // bulan saat ini untuk forecast/pemesanan
                ...
                if (rows.length === 0) {
                    ...
                    rows.forEach(item => {
                        ...
                        const pemesanan = parseFloat(item[`pesan_${bulanKeys[idxForecast]}`]) || 0;

                        const rata = ((val1 + val2 + val3 + val4) / 4);
                        const selisih = rata - pemesanan;

                        const warnClass = (rata === 0) ? 'table-warning' : '';
                        ...
                        html += `
                            <tr class="${warnClass}">
                                ...
                                <td>${todesimal(pemesanan)}</td>
                                <td>${todesimal(selisih)}</td>
                                <td>0</td>
