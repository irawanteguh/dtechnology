        ...
        function datainsight($tahun) {
            ...
            $query = "
                        ...
                        SELECT DATE_FORMAT(tanggal, '%M') AS periode,
                            ...
                            (select coalesce(sum(k_urj+k_uri+k_arj+k_ari+k_brj+k_bri+lain),0) from dt01_report_income_dt where active='1' and org_id='10c84edd-500b-49e3-93a5-a2c8cd2c8524' and date_format(date,'%m.%Y')=date_format(tanggal,'%m.%Y'))kunjungantotalrsms,
                            (select coalesce(sum(k_urj+k_uri+k_arj+k_ari+k_brj+k_bri+lain),0) from dt01_report_income_dt where active='1' and org_id='d5e63fbc-01ec-4ba8-90b8-fb623438b99d' and date_format(date,'%m.%Y')=date_format(tanggal,'%m.%Y'))kunjungantotalrsiabm,
                            (select coalesce(sum(k_urj+k_uri+k_arj+k_ari+k_brj+k_bri+lain),0) from dt01_report_income_dt where active='1' and org_id='a4633f72-4d67-4f65-a050-9f6240704151' and date_format(date,'%m.%Y')=date_format(tanggal,'%m.%Y'))kunjungantotalrst,
