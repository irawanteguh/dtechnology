        ...
        <div class="card rounded bgi-no-repeat bgi-position-x-end bgi-size-cover"
            ...
            <div class="card-body pt-9 pb-0">
                <div class="d-flex flex-wrap flex-sm-nowrap mb-5">
                    <div>
                        <h1 class="text-white">Summary Laporan Piutang</h1>
                ...
                <div class="d-flex overflow-auto min-h-30px">
                    <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder flex-nowrap">
                        <?php foreach ($hospitalList as $id => $name): ?>
                            <li class="nav-item">
                                <a class="nav-link <?= ($id === $firstKey) ? 'active' : '' ?>" data-bs-toggle="tab" href="#piutang_<?= $id ?>" id="tab_<?= strtolower(str_replace(' ', '', $name)) ?>" style="color:#fff;"><?= $name ?></a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
...
<div class="tab-content mt-5" id="mutasiTabContent">
    <?php foreach ($hospitalList as $id => $name): ?>
        ...
        <div class="tab-pane fade <?= $isActive ?>" id="piutang_<?= $id ?>" role="tabpanel">
            <div class="row gy-5 g-xl-8 mb-xl-8">
                <div class="col-xl-12 border">
                                ...
                                <table class="table align-middle table-row-dashed fs-8 gy-2">
                                    <thead>
                                        <tr class="fw-bolder align-middle bg-primary text-white">
                                            ...
                                            <th>Jenis Tagihan</th>
                                            <th>Rekanan / Catatan</th>
                                            <th class="text-center">Tanggal Tagihan</th>
                                            <th class="text-end">Tagihan</th>
                                            <th class="text-end">Terbayar</th>
                                            <th class="pe-4 rounded-end text-end">Sisa Tagihan</th>
                                        </tr>
                                    </thead>
                                    <tbody class="text-gray-600 fw-bold" id="resultdatapiutang_<?= $id ?>"></tbody>
                                </table>
                            </div>
                        </div>
                    </div>
