                    ...
                    "
                        select a.no_pemesanan, judul_pemesanan, note, invoice, invoice_no, method, cito, attachment_note, subtotal, harga_ppn, total, status, status_vice, status_dir, attachment, date_format(a.created_date, '%d.%m.%Y %H:%i:%s')tglbuat,
                            ...
                            case 
                                ...
                                when status = '7' then 'badge-light-danger|Invoice Cancelled Manager'
                                when status = '8' then 'badge-light-info|Invoice Approval Manager'
                                when status = '9' then 'badge-light-danger|Invoice Cancelled Vice Director'
                                when status = '10' then 'badge-light-info|Invoice Approval Vice Director'
                                when status = '11' then 'badge-light-danger|Invoice Cancelled Director'
                                when status = '12' then 'badge-light-info|Invoice Approval Director'
                                when status = '13' then 'badge-light-danger|Invoice Cancelled Finance'
                                when status = '14' then 'badge-light-info|Invoice Approval Finance'
                                when status = '15' then 'badge-light-success|Payment Success'
                                when status = '16' then 'badge-light-success|File Transfer Available'
