<?php
    ...
    class Paymentrequest extends CI_Controller{
        ...
        public function decline(){
            $status="and   a.department_id in (select department_id from dt01_gen_department_ms where org_id=a.org_id and active='1' and user_id='".$_SESSION['userid']."') and a.status in ('8')";
            $result = $this->md->datarequest($_SESSION['orgid'],$status);
        ...
        public function approve(){
            $status="and   a.department_id in (select department_id from dt01_gen_department_ms where org_id=a.org_id and active='1' and user_id='".$_SESSION['userid']."') and a.status in ('9')";
            $result = $this->md->datarequest($_SESSION['orgid'],$status);
