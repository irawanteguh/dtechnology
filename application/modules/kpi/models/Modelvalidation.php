...
SELECT 
    ...
    (SELECT level 
     ...
     WHERE org_id = d.org_id 
       AND active = '1' 
       AND level_id = p.level_fungsional) AS fungsionalprimary,
    (SELECT CONCAT(name,' ',area) 
     FROM dt01_hrd_klinis_ms 
     WHERE active = '1' 
       AND klinis_id = u.klinis_id) AS klinis,
    ...
    ROUND(COALESCE(a.total_nilai,0) / NULLIF(a.total_komponen,0) * COALESCE(ea.nilai,0) * 10, 0) +
    ROUND(CASE WHEN act.jmldisetujui > u.hours_month THEN u.hours_month ELSE act.jmldisetujui END / u.hours_month * COALESCE(ev.nilai,0) * 100, 0) AS resultkpi
FROM (
    SELECT a.org_id, user_id, position_primary, atasan_id, position_id
    FROM dt01_hrd_position_dt a
    WHERE a.active = '1'
    ...
    SELECT a.org_id, user_id, 'X' AS position_primary, atasan_id, 
           (SELECT position_id 
            ...
            WHERE active = '1' 
              AND status = '1' 
              AND org_id = a.org_id 
              AND user_id = a.user_id) AS position_id
    FROM dt01_hrd_activity_dt a
    WHERE a.active = '1'
      AND a.status = '0'
      AND a.org_id = '".$orgid."'
